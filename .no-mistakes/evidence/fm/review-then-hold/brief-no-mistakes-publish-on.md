...
1. First action: create your branch: `git checkout -b fm/demo-on --`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions),
   ...
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to `/var/folders/36/2_6kg25x1wj4bnsb4mhgc7th0000gn/T/tmp.iiZhdbuymg/data/demo-on/nm-<run>-findings.txt`, then report the gate with
   `needs-decision [at=<epoch>] [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=/var/folders/36/2_6kg25x1wj4bnsb4mhgc7th0000gn/T/tmp.iiZhdbuymg/data/demo-on/nm-<run>-findings.txt`
...
Firstmate steers you through durable message files in '/var/folders/36/2_6kg25x1wj4bnsb4mhgc7th0000gn/T/tmp.iiZhdbuymg/state/demo-on.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/var/folders/36/2_6kg25x1wj4bnsb4mhgc7th0000gn/T/tmp.iiZhdbuymg/state/demo-on.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/var/folders/36/2_6kg25x1wj4bnsb4mhgc7th0000gn/T/tmp.iiZhdbuymg/state/demo-on.inbox'/NNN.msg '/var/folders/36/2_6kg25x1wj4bnsb4mhgc7th0000gn/T/tmp.iiZhdbuymg/state/demo-on.inbox'/handled/`.
...
Ship branch: fm/demo-on
Publish authorization: on
...
The task is complete only when committed on your branch `fm/demo-on`.
...
3. Confirm with `git log` that `fm/demo-on` now carries every fix commit the run made, whether or not step 2 was needed.
...
When the run ends failed, fix what it names on `fm/demo-on`, commit, and start the review pass again; iterate until its outcome is passed, passed-with-skips, or passed-with-override and step 3 holds.
...
## Publish
Publishing is authorized for this task: after the clean review pass, publish from `fm/demo-on` as follows without waiting for a further instruction.
Start a publishing run on `fm/demo-on` with the same `--intent`, skipping only the steps the clean review pass already ran: `no-mistakes axi run --intent "<intent>" --skip review,test,document,lint`.
