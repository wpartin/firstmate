...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, the optional captain's log under `data/log/` ([`captains-log.md`](captains-log.md)), and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
See [`captains-log.md`](captains-log.md) for the location setting, the optional ticket, people, and redaction patterns, and what the log renders.
All four files are local, gitignored, per home, and not inherited by second mate homes.
...
An absent or blank file changes nothing, while a present path that is not a readable regular file, or text carrying its own `Delivery contract: mode=` or `Publish authorization:` line, stops the scaffold before anything is written.
