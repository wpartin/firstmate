#!/usr/bin/env bash
# Behavior tests for bin/fm-pr-quality-check.sh.
#
...
CHECK="$ROOT/bin/fm-pr-quality-check.sh"
TMP_ROOT=$(fm_test_tmproot fm-pr-quality-check)

# The action is a private per-home setting; every fixture names an invented one.
mkdir -p "$TMP_ROOT/config"
printf 'example-org/pr-guard\n' > "$TMP_ROOT/config/pr-quality-action"
...
write_workflow() {
  ...
  cat > "$dir/.github/workflows/pr-guard.yaml" <<'YAML'
name: PR Guard
on:
  pull_request:
    types: [opened, edited]
...
test_clean_branch_and_body_pass() {
  ...
  pass "fm-pr-quality-check: a clean branch and body pass with every rule measured"
}
...
test_every_rule_reports_measurement_and_limit() {
  ...
  pass "fm-pr-quality-check: every rule prints its measured number against its limit"
}
...
test_changed_lines_breach_is_named() {
  ...
  pass "fm-pr-quality-check: a changed-line breach is named"
}
...
test_changed_files_breach_is_named() {
  ...
  pass "fm-pr-quality-check: a changed-file breach is named"
}
...
test_description_length_breach_is_named() {
  ...
  pass "fm-pr-quality-check: a description-length breach is named"
}
...
test_empty_description_breach_is_named() {
  ...
  pass "fm-pr-quality-check: an empty description is named"
}
...
test_emoji_breach_is_named() {
  ...
  pass "fm-pr-quality-check: an emoji breach is named"
}
...
test_emoji_counts_the_title_when_given() {
  ...
  pass "fm-pr-quality-check: the emoji rule counts the title, and says so when it has none"
}
...
test_blocked_term_breach_is_named() {
  ...
  pass "fm-pr-quality-check: a blocked term is named"
}
...
test_blocked_term_inside_an_html_comment_is_not_a_breach() {
  ...
  pass "fm-pr-quality-check: a blocked term inside an HTML comment is not a breach"
}
...
test_code_reference_breach_is_named() {
  ...
  pass "fm-pr-quality-check: a code-reference breach is named"
}
...
test_blocked_path_breach_is_named() {
  ...
  pass "fm-pr-quality-check: a blocked-path breach is named"
}
...
YAML
  ...
  pass "fm-pr-quality-check: a file matching two blocked patterns counts once"
}
...
test_all_breaches_are_reported_in_one_run() {
  ...
  pass "fm-pr-quality-check: every simultaneous breach is named in one run"
}
...
YAML
  ...
  pass "fm-pr-quality-check: a project with no such check is reported as not applicable"
}
...
test_project_with_no_workflows_at_all_is_not_applicable() {
  ...
  pass "fm-pr-quality-check: a project with no workflows at all is not applicable"
}

test_home_without_an_action_is_not_applicable() {
  local dir="$TMP_ROOT/noaction" out rc=0
  mkdir -p "$dir" "$TMP_ROOT/empty-config"
  write_workflow "$dir"
  out=$(FM_CONFIG_OVERRIDE="$TMP_ROOT/empty-config" "$CHECK" --project "$dir" --body "$TMP_ROOT/clean-body.md" 2>&1) || rc=$?
  expect_code 0 "$rc" "a home with no configured action must exit successfully"
  assert_contains "$out" "not applicable" "a home with no configured action must say so"
  out=$(FM_CONFIG_OVERRIDE="$TMP_ROOT/empty-config" "$CHECK" --project "$dir" --print-limits 2>&1)
  assert_contains "$out" "applicable=no" "--print-limits must report not applicable without an action"
  pass "fm-pr-quality-check: a home with no configured action is not applicable"
}

test_action_flag_overrides_the_setting() {
  local dir="$TMP_ROOT/actionflag" out rc=0
  mkdir -p "$dir"
  write_workflow "$dir"
  out=$("$CHECK" --project "$dir" --action other-org/other-guard --print-limits 2>&1) || rc=$?
  expect_code 0 "$rc" "an --action no workflow uses must exit successfully"
  assert_contains "$out" "applicable=no" "--action must replace the configured action"
  rc=0
  out=$(FM_CONFIG_OVERRIDE="$TMP_ROOT/empty-config" "$CHECK" --project "$dir" --action example-org/pr-guard@v1 --print-limits 2>&1) || rc=$?
  expect_code 0 "$rc" "--action with a version suffix must be accepted"
  assert_contains "$out" "applicable=yes" "--action must select the step it names"
  rc=0
  out=$("$CHECK" --project "$dir" --action 'bad name' --print-limits 2>&1) || rc=$?
  expect_code 2 "$rc" "a malformed action reference must be refused"
  pass "fm-pr-quality-check: --action overrides the configured action"
}
...
YAML
  ...
  pass "fm-pr-quality-check: an unparseable limit fails closed"
}
...
YAML
  ...
  pass "fm-pr-quality-check: an unparseable workflow fails closed"
}
...
YAML
  ...
  pass "fm-pr-quality-check: a limit given as a template expression fails closed"
}

test_two_pr_quality_steps_fail_closed() {
...
YAML
  ...
  pass "fm-pr-quality-check: more than one configured step fails closed"
}
...
YAML
  ...
  pass "fm-pr-quality-check: limits are read from the project rather than hardcoded"
}
...
YAML
  ...
  pass "fm-pr-quality-check: unset inputs fall back to labeled action defaults"
}
...
jobs:
  pr-guard:
    steps:
      - uses: example-org/pr-guard@v9.9.9
        with:
          max-changed-lines: 5000
YAML
  commit_all "$dir" work
  ...
  pass "fm-pr-quality-check: an unfamiliar action version warns but still measures"
}
...
test_missing_inputs_are_reported_as_unmeasured() {
  ...
  pass "fm-pr-quality-check: a rule it cannot measure is reported, never treated as passing"
}
...
test_usage_errors_are_refused() {
  ...
  pass "fm-pr-quality-check: bad usage is refused rather than measured around"
}
...
test_print_limits_is_machine_readable() {
  ...
  pass "fm-pr-quality-check: --print-limits is machine readable for both outcomes"
}
...
test_help_includes_entire_header() {
  ...
  pass "fm-pr-quality-check: --help documents what it approximates"
}

test_script_parses() {
  local out rc
  out=$(bash -n "$ROOT/bin/fm-pr-quality-check.sh" 2>&1); rc=$?
  expect_code 0 "$rc" "bash -n bin/fm-pr-quality-check.sh must parse cleanly (got: $out)"
  [ -z "$out" ] || fail "bash -n bin/fm-pr-quality-check.sh emitted unexpected output: $out"
  pass "fm-pr-quality-check: bash -n succeeds"
}
...
test_commit_message_length_breach_is_named() {
  ...
  pass "fm-pr-quality-check: an oversized commit message is named as a breach"
}
...
test_commit_message_at_the_limit_passes() {
  ...
  pass "fm-pr-quality-check: a commit message exactly at the limit passes"
}
...
test_base_branch_commit_messages_are_not_measured() {
  ...
  pass "fm-pr-quality-check: only the commits this branch adds are measured"
}
...
test_commit_message_rule_is_skipped_without_a_base() {
  ...
  pass "fm-pr-quality-check: the commit-message rule is skipped, not assumed, without a base ref"
}
...
YAML
  ...
  pass "fm-pr-quality-check: a commit-message limit of 0 disables the rule"
}
